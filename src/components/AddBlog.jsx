...
const AddBlog = ({ isOpen, onClose , fetchBlogs}) => {
  ...
  const removeTag = (tagToRemove) => {
    const tagsArray = formData.tags.split(',').filter(tag => tag !== tagToRemove);
    setFormData({
      ...formData,
      tags: tagsArray.join(',')
    });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (!formData.title.trim() || !formData.content.trim() || !formData.tags.trim() || !formData.image) {
        setMessage({ 
          text: 'Please fill in all required fields: Title, Content, Tags, and Image', 
          type: 'error' 
        });
  ...
  const tagsArray = formData.tags ? formData.tags.split(',') : [];
        ...
        <form onSubmit={handleSubmit} className="p-6 overflow-y-auto max-h-[calc(90vh-140px)]">
          ...
          <div className="mb-6">
            ...
            <div className="flex items-center justify-center w-full">
              <label className={`flex flex-col items-center justify-center w-full h-40 border-2 border-dashed rounded-lg cursor-pointer transition-all duration-300 hover:border-blue-400 hover:bg-blue-50 dark:hover:bg-gray-700 ${imagePreview ? 'border-blue-400' : 'border-gray-300 dark:border-gray-600'}`}>
                {imagePreview ? (
